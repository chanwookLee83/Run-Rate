...
let DB = { projects: [] };
const APP_VERSION = 'v16'; // 배포 버전 표기 (sw.js 캐시 버전과 함께 올림)
let state = {
...
// 4M(설비/사람/방법/자재) 사유가 태깅된 랩은 이상치로 보고 표준 C/T·Rate%·CAPA 산출에서 제외한다.
function isNormalCycle(c){ return !(c.reasons && c.reasons.length>0); }

function computeRate(proj, processId){
  const proc = proj.processes.find(p=>p.id===processId);
  const cycles = getCycles(proj, processId);
  const normalCycles = cycles.filter(isNormalCycle);
  if(!proc || normalCycles.length===0) return { avgCt:null, uph:null, ratePct:null, n:0, nTotal:cycles.length, minCt:null, maxCt:null, stdCt:null };
  const cts = normalCycles.map(c=> Number(c.ct)).filter(v=>!isNaN(v) && v>0);
  if(cts.length===0) return { avgCt:null, uph:null, ratePct:null, n:0, nTotal:cycles.length, minCt:null, maxCt:null, stdCt:null };
...
function capaInsight(ratePct, n, hasTargetCt){
  if(ratePct===null){
    if(hasTargetCt===false) return { tone:'warn', title:'목표 C/T 미설정', msg:'"공정 등록" 탭에서 이 공정의 목표 C/T(초)를 입력하면 Rate%/CAPA 갭 분석이 표시됩니다.' };
    return { tone:'warn', title:'측정 데이터 부족', msg:'사이클타임 5건 이상을 먼저 측정하세요.' };
  }
...
// 측정 기록 테이블: 4M(설비/사람/방법/자재) 체크박스는 renderAnalysisTab / refreshCycleTableOnly에서 공통으로 사용
function renderCycleTableBody(cycles){
...
function bindCycleTableEvents(container, projId){
  container.querySelectorAll('[data-del-cycle]').forEach(el=>{
    el.addEventListener('click', async ()=>{
      try{ await fb.deleteDoc(fb.doc(cyclesCol(projId), el.dataset.delCycle)); }
...
function renderAnalysisTab(proj){
  ...
  <div class="kpi-strip cols-6" id="analysis-kpi-strip">
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-sub">${r.nTotal>r.n? `정상 ${r.n} / 전체 ${r.nTotal}건` : (cycles.length>0? '최근 '+fmtDate(cycles[0].ts) : '측정 없음')}</div>
    </div>
  </div>
  ...
  <div class="panel" id="cycle-record-panel">
    ...
    <div class="panel-body" style="padding:0; max-height:360px; overflow-x:auto; overflow-y:auto;">
      ${renderCycleTableBody(cycles)}
    </div>
  </div>`;
}
...
document.getElementById('btn-save-process').addEventListener('click', async ()=>{
  ...
  rows.forEach(r=>{
    ...
    const ctInp = r.querySelector('.proc-ct-inp');
    if(!name){ ctInp.style.borderColor=''; return; }
    const targetCt = ctInp.value ? Number(ctInp.value) : null;
    if(!targetCt || targetCt<=0){
      missingCt.push(name);
      ctInp.style.borderColor = 'var(--red)';
    } else {
      ctInp.style.borderColor = '';
    }
    names.push({
      name,
  ...
  if(missingCt.length>0){ toast(`목표 C/T(초)를 입력해야 저장할 수 있습니다: ${missingCt.join(', ')}`, 'error'); return; }
...
function attachContentEvents(proj){
  ...
  const recordPanelEl = document.getElementById('cycle-record-panel');
  if(recordPanelEl) bindCycleTableEvents(recordPanelEl, proj.id);
